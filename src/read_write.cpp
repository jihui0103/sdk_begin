...
//   int dxl_goal_position[2] = {DXL_MINIMUM_POSITION_VALUE, DXL_MAXIMUM_POSITION_VALUE};         // Goal position
  ...
  uint32_t dxl_goal_position_cos_1 = 0;
...
while(1)
  { 
    dxl_goal_position_cos_1 = (((3072-1024) / 2) * (1-cos(2 * 3.14 * t / T))) + 1024;
    // Write goal position
    dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, DXL_ID, ADDR_PRO_GOAL_POSITION, dxl_goal_position_cos_1, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS)
    {
      packetHandler->getTxRxResult(dxl_comm_result);
    }
    else if (dxl_error != 0)
    {
      ...
      printf("[ID:%03d] GoalPos:%03d  PresPos:%03d t:%f\n", DXL_ID, dxl_goal_position_cos_1, dxl_present_position, t);
